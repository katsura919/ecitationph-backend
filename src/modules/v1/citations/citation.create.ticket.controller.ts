    ...
    try {
        ...
        if (!enforcerId) {
            return res.status(401).json({
                success: false,
                error: 'Enforcer ID not provided. Please authenticate or provide x-enforcer-id header',
            });
        }
        ...
        const violationItems = await Promise.all(
            violations.map(async violation => {
                ...
                if (
                    ...
                    console.log(
                        `[Fixed Fine] Processing ${violation.code} as FIXED`
                    );
                    ...
                    offenseCount =
                        previousCitations.reduce((count, citation) => {
                            const violationCount = citation.violations.filter(
                                (v: any) =>
                                    v.violationId &&
                                    v.violationId.toString() === violationIdStr
                            ).length;
        ...
        await citation.populate([
            ...
            {
                path: 'vehicleId',
                select: 'plateNo vehicleType make vehicleModel color ownerFirstName ownerMiddleName ownerLastName',
            },
            {
                path: 'issuedBy',
                select: 'badgeNo name',
        ...
        console.error(
            `[Citation Create] Request ID: ${requestId} - Error:`,
            error
        );
