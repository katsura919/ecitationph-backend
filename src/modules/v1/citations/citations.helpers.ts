...
export async function getOverdueCitations(
    ...
    return CitationModel.find({
        dueDate: { $lt: now },
        status: {
            $in: [CitationStatus.PENDING, CitationStatus.PARTIALLY_PAID],
        },
        isVoid: false,
    }).sort({ dueDate: 1 });
}
        ...
        { $match: matchStage },
        {
            $group: {
                ...
                pending: {
                    $sum: {
                        $cond: [
                            { $eq: ['$status', CitationStatus.PENDING] },
                            1,
                            0,
                        ],
                    },
                ...
                overdue: {
                    $sum: {
                        $cond: [
                            { $eq: ['$status', CitationStatus.OVERDUE] },
                            1,
                            0,
                        ],
                    },
                ...
                contested: {
                    $sum: {
                        $cond: [
                            { $eq: ['$status', CitationStatus.CONTESTED] },
                            1,
                            0,
                        ],
                    },
