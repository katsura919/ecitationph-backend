...
export const updateOrdinance = async (req: Request, res: Response) => {
    try {
        ...
        // Create new version
        const newVersion = await ordinance.createNewVersion(
            updates,
            req.user?.id
        );
...
export const deleteOrdinance = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message:
                'Ordinance and associated violations deactivated successfully',
            data: ordinance,
        });
    } catch (error: any) {
...
export const removeViolationFromOrdinance = async (
    ...
    try {
        ...
        await ordinance.removeViolation(
            new mongoose.Types.ObjectId(violationId)
        );
